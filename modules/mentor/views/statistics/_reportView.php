...
<img src="/img/another/gud.jpg" class="print-img"> 
...
<h3 style="font-family: 'Calibri'">Отчет о деятельности группы <?=$group_number ?></h3>
<h4 style="font-family: 'Calibri'">Преподаватель: <?=$mentor->first_name?> <?=$mentor->last_name?> <?=$mentor->middle_name?></h4>
<table class="table">
		<tr>
			<td class="group-stat-heading">#</td>
			<td class="group-stat-heading"><label id="center">ФИО студента</label></td>
			<?php if($groupStat['works'] != NULL):?>
				...
				<?php endforeach;?>
			<?php else:?>
				<td class="group-stat-heading">В данный момент группой не пройдено ни одной темы</td>
			<?php endif;?>
			<td class="group-stat-heading">Средняя оценка</td>
