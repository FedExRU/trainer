			...
			<tr class="container-center-log-heading">
				...
				<?php for ($i = 0; $i < count($newLog); $i++): ?>
					<tr>
						...
						<td><?=$newLog[$i]->date?></td>
						<td>
							<?= Html::a('Отметить', ['/mentor/log/check', 'log_id' => $newLog[$i]->log_id]) ?>
							<?= Html::a('Удалить', ['/mentor/log/delete', 'log_id' => $newLog[$i]->log_id], ['class'=>'', 'id' => '']) ?>
						</td>
					</tr>
				<?php endfor; ?>
		</table>
		...
		<table class="table">
			...
			<?php for ($i = 0; $i < count($oldLog); $i++): ?>
				<tr>
					...
					<td><?=$oldLog[$i]->text?></td>
					<td><?=$oldLog[$i]->date?></td>
					<td><?= Html::a('Удалить', ['/mentor/log/delete', 'log_id' => $oldLog[$i]->log_id], ['class'=>'', 'id' => '']) ?></td>
				</tr>
			<?php endfor; ?>
		</table>
	</div>
	<div class="container-center-log-buttons">
		<?= Html::a('Отметить все', ['/mentor/log/checkall', 'user_id' => Yii::$app->user->getId()], ['class'=>'main-menu-button', 'id' => 'check-log-button']) ?>
		<a href="/mentor" class="main-menu-button" id="back-button-satistics">Назад</a>
	</div>
